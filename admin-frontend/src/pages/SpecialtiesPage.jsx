...
const modalFields = [
  { name: 'name', label: '名称', type: 'text', placeholder: '请输入特产名称', required: true },
  ...
  { name: 'rating', label: '评分', type: 'number', placeholder: '评分', defaultValue: 4.5 },
...
function SpecialtiesPage() {
  ...
  const handleSubmit = async (data) => {
    if (editingItem) await specialtyApi.update(editingItem.id, data);
    else await specialtyApi.create(data);
    fetchItems();
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900">地方特产管理</h1>
        <button onClick={() => { setEditingItem(null); setModalOpen(true); }} className="admin-btn-primary flex items-center gap-2">
          <Plus className="w-4 h-4" />添加特产
        </button>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {items.length === 0 ? (
          <div className="col-span-full p-12 text-center text-gray-500 admin-card">暂无特产数据</div>
        ) : items.map((item) => (
      ...
      <ContentModal isOpen={modalOpen} onClose={() => setModalOpen(false)} onSubmit={handleSubmit} title={editingItem ? '编辑特产' : '添加特产'} fields={modalFields} initialData={editingItem} />
        ...
        {deleteConfirm && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative bg-white rounded-2xl shadow-2xl p-6 max-w-md w-full">
              ...
              <p className="text-gray-600 mb-6">确定要删除特产「<span className="font-medium">{deleteConfirm.name}</span>」吗？</p>
